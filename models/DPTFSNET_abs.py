...
class transformer(tf.keras.Model):
    ...
    def call(self,inputs):
        shape=inputs.shape
        x=tf.reshape(inputs,shape=[-1,shape[-2],shape[-1]])
        x=self.LSTM_embedder(x)
        x=self.attention_layer(x,x)
        x=tf.reshape(x,shape=[-1,shape[-3],shape[-2],shape[-1]])
...
class DPTPM(tf.keras.Model):
    def __init__(self,filter,input_shape,units,features,frames,heads,latent_dim,dense_units):
        super().__init__()
        self.shape=None
        self.conv_layer1=tf.keras.layers.Conv2D(filters=filter,kernel_size=[1,1],strides=1,padding="same")
        self.DPT=DPT(input_shape,units,features,frames,heads,latent_dim,dense_units)
        self.conv_layer2=tf.keras.layers.Conv2D(filters=filter,kernel_size=[1,1],strides=1,padding="same")
...
class DPTFSNET(tf.keras.Model):
    def __init__(self,filter,layers,input_shape,units,features,frames,heads,latent_dim,dense_units):
        ...
        self.DPTPM=DPTPM(filter,input_shape,units,features,frames,heads,latent_dim,dense_units)
...
class model(tf.keras.Model):
    def __init__(self,filter,layers,input_shape,units,features,frames,heads,latent_dim,dense_units):
        super().__init__()
        self.shape=None
        self.model=DPTFSNET(filter,layers,input_shape,units,features,frames,heads,latent_dim,dense_units)
        
    def call(self,inputs):
        shape=tf.shape(inputs)
        x=tf.reshape(inputs,shape=[shape[0]*shape[1],shape[2],shape[3]])
        x=self.model(x)
...
class trainer():
    def __init__(self,
        ...
        self.units=units
        self.features=features
        self.frames=LSTM_embedder_frames
        self.heads=heads
    ...
    def pack_model(self,input_shape,optimizer,loss,metrics):
        self.input_shape=input_shape
        SEmodel=model(filter=self.filters,
                      layers=self.layers,
